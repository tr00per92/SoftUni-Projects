namespace ShoppingCenter.Tests
{
    ...
    public class UnitTestsShoppingCenter
    {
        ...
        private static void ExecuteTest(string inputFileName, string outputFileName)
        {
            Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;

            var shoppingCenter = new ShoppingCenter();
            var inputCommands = File.ReadAllLines(@"..\..\JudgeTests\" + inputFileName);
            var output = new StringBuilder();
            var commandsCount = int.Parse(inputCommands[0]);
            ...
            var expectedOutput = File.ReadAllText(@"..\..\JudgeTests\" + outputFileName);
